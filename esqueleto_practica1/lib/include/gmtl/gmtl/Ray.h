...
public:
    ...
    */
   Ray( const Point<DATA_TYPE, 3>& origin, const Vec<DATA_TYPE, 3>& dir, float maxLength = FLT_MAX )
	   : mOrigin(origin), mDir(dir), mMaxLength(maxLength)
   {}
   ...
   void setMaxLength(float ml) { mMaxLength = ml;}
