...
/**
 * Compute the multiplication of v1 and v2 component to component and return the result
 *
 * Example:
 * @code
 *     Vec3f b(1.,0.,0.), c(0.,1.,0.);
 ...
 * @code
 *     Vec3f b(1.,0.,0.), c(0.,1.,0.);
 *     Vec3f a(b[0]*c[0],b[1]*c[1],b[2]*c[2]);
 * @endcode
 * 
 * @pre v1 and v2 are 3-D vectors
 *
 ...
 * @param v2   the second vector
 *
 * @return  the component per component multiplication of v1 and v2
 */
template<class DATA_TYPE>
inline Vec<DATA_TYPE, 3> operator*( const Vec<DATA_TYPE, 3>& v1,
                         const Vec<DATA_TYPE, 3>& v2 )
{
    Vec<DATA_TYPE, 3> result(v1[0]*v2[0],v1[1]*v2[1],v1[2]*v2[2]);
    return(result);
}
